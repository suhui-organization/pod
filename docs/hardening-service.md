...
| 文件 | 给谁看 |
|------|--------|
| `report.md` | 客户：执行摘要 → 范围与方法 → 结论与待办（每条带可执行命令）→ 覆盖边界 → 五份子报告 → 如何验证 → 产物清单与声明 |
| `findings.json` | 客户的自查工具 / 下次审计做 diff（含交付元信息与多 harness 计数） |
| `policy-draft.json` | 直接可以 `pod lint` + `pod serve` 的最小权限草稿 |
| `harness-scan.md` / `harness-findings.json` | 16 类 harness 的完整扫描明细（客户/审计方要复核时看这一份，不用重跑） |
| `evidence.json` | 审计链证据包（含自校验哈希） |
| `manifest.json` | 每份产物的 sha256——**交付物本身可自证没被改过** |

### 1.2.1 交付物是对客户说的，不是对操作者说的

同一个 `pod guard` 内核在两个场合说两种话，这是刻意的：

| 场合 | 说什么 | 不说什么 |
|------|--------|----------|
| `pod guard scan`（操作者，免费） | "先做这三件事"+ 每条的可执行命令 + 交付入口 | 客户交付物才有的范围声明、验证指引 |
| `pod harden`（客户交付物） | 执行摘要、范围与方法、覆盖边界（看不到什么）、每条待办的命令、如何独立复验 | 工具自己的推销话术、"先做这三件事"这类面向操作的提示 |

客户拿到报告后不需要装 pod 就能验一部分：`manifest.json` 里的 sha256 可以用任何
工具重算；装了 pod 的一方可以直接跑 `pod harden --verify <目录>` 逐文件核对。
**报告被改动一个字节，这一步就会失败并指出是哪个文件**——这是"证据优先"与
"口头保证"之间唯一的区别。

交付元信息（`--client` / `--auditor` / `--engagement`）会写进报告封面、`findings.json`
与 `manifest.json`，让同一客户多次审计的报告能对上工单号。

### 1.2.2 一行命令跑一遍（客户侧）

```bash
pod harden --out ~/audit-<客户>-<日期> \
  --client "<客户名>" --auditor "<你的名字/公司>" --engagement "<工单号>"
pod harden --verify ~/audit-<客户>-<日期>     # 交付前自检：哈希是不是都对得上
```
