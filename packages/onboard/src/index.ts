 ...
 *
 * 为什么放在包里而不是 apps/cli：控制台的「接管」按钮与 `pod onboard` 必须走
 * 同一条写路径。各写一份的后果是"网页改的配置与 CLI 改的配置不一致"，
 * 而这类不一致只有在出事复盘时才会被发现。
 */
...
export interface OnboardApplyOptions {
  ...
  podBin?: string;
  /**
   * 覆盖某个 agent 的包装策略路径（默认 <policyDir>/onboard-<agent>.json）。
   *
   * 控制台纳管时用它指向已有的零权限策略：不新建 allow-all 包装策略，
   * 也让"一个 agent 一份策略"成立。默认行为不变。
   */
  policyPathFor?: (agent: string) => string;
  /**
   * true = 目标策略文件已存在时不覆盖。
   *
   * 包装策略的模板是 allow:['*']（只录不拦时期无所谓）。但如果用户已经有一份
   * **零权限**策略，覆盖它等于把一个 fail-closed 的策略换成 fail-open 的——
   * 一旦哪天有人去掉 --record-only，行为完全不同。宁可留着用户的。
   */
  keepExistingPolicy?: boolean;
}
...
export function applyOnboard(targets: OnboardTarget[], opts: OnboardApplyOptions): OnboardResult {
  ...
  const policyPathOf = (agent: string): string =>
    opts.policyPathFor?.(agent) ?? join(opts.policyDir, `onboard-${agent}.json`);

  for (const [agent, servers] of byAgent) {
    const path = policyPathOf(agent);
    policies.push(path);
    if (!opts.dryRun) {
      mkdirSync(opts.policyDir, { recursive: true });
      if (opts.keepExistingPolicy === true && existsSync(path)) continue;
  ...
  for (const t of targets) {
    ...
    const policyPath = policyPathOf(t.agent);
    if (opts.dryRun) {
