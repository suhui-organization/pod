...
# 链接钉在发布版上（可复现）：想跟主干就把 v0.4.1 换成 main
curl -fsSL https://gitee.com/suhuisoftwares/pod/raw/v0.4.1/scripts/install.sh | sh
# 主源连不上时用 GitHub 镜像（同一个脚本）：
# curl -fsSL https://raw.githubusercontent.com/suhui-organization/pod/v0.4.1/scripts/install.sh | sh
pod --help
```

The installer clones `v0.4.1` to `~/.pod/src`, builds, and puts `pod` in `~/.local/bin`. Override with `POD_SRC`, `POD_BIN_DIR`, `POD_REPO_URL`, `POD_VERSION`（`POD_VERSION=main` 跟主干）。
...
```bash
git clone --branch v0.4.1 --depth 1 https://gitee.com/suhuisoftwares/pod.git && cd pod
pnpm install && pnpm build
