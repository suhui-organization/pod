...
class McpServer(Base):
    """租户级 MCP 服务器(能力中心管理,取代/兼容旧 mcp_commands 文本)。

    P1 增强:transport 类型(stdio|streamable-http)+ url + headers/env(凭据服务端保存,
    回显脱敏);stdio 走命令启动,streamable-http 走 url + headers(如 Google Stitch)。
    """
    ...
    command: Mapped[str] = mapped_column(String(512), default="")  # stdio 启动命令(如 python3 -m akshare_mcp_server)
    transport: Mapped[str] = mapped_column(String(16), default="stdio")  # stdio | streamable-http
    url: Mapped[str] = mapped_column(String(512), default="")  # streamable-http 端点
    headers_json: Mapped[str] = mapped_column(Text, default="{}")  # streamable-http 请求头(敏感字段服务端保存,回显脱敏)
    env_json: Mapped[str] = mapped_column(Text, default="{}")  # stdio 环境变量(敏感字段服务端保存,回显脱敏)
    timeout_s: Mapped[int] = mapped_column(Integer, default=60)
    source: Mapped[str] = mapped_column(String(16), default="manual")  # manual | plugin
    enabled: Mapped[bool] = mapped_column(Boolean, default=True)  # 工具级开关:停用后对话不装配
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)


class CustomSkill(Base):
    """租户级自定义 Skill(场景):system prompt + 工具白名单;内置四场景来自内核。"""

    __tablename__ = "custom_skills"

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    tenant_id: Mapped[int] = mapped_column(ForeignKey("tenants.id"), index=True)
    name: Mapped[str] = mapped_column(String(32))  # 场景名(唯一,租户内)
    description: Mapped[str] = mapped_column(String(255), default="")
    system_prompt: Mapped[str] = mapped_column(Text, default="")
    tools_json: Mapped[str] = mapped_column(Text, default="[]")  # 工具名白名单
    source: Mapped[str] = mapped_column(String(16), default="manual")  # manual | plugin
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)


class Plugin(Base):
    """插件:打包的能力单元(内含 MCP 服务器列表 + Skill 列表),安装即注册。

    P0 宿主协议扩展列:plugin_id(包 id,租户内唯一)/manifest_json/code_text/
    ui_slots_json/enabled/scope——承载"一切皆插件"的安装面与 UI 槽位声明。
    """
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    tenant_id: Mapped[int] = mapped_column(ForeignKey("tenants.id"), index=True)
    plugin_id: Mapped[str] = mapped_column(String(64), default="")  # 插件包 id(租户内唯一)
    name: Mapped[str] = mapped_column(String(64))
    description: Mapped[str] = mapped_column(String(255), default="")
    version: Mapped[str] = mapped_column(String(32), default="1.0.0")
    mcp_commands_json: Mapped[str] = mapped_column(Text, default="[]")  # [{name, command}]
    skills_json: Mapped[str] = mapped_column(Text, default="[]")  # [{name, description, system_prompt, tools}]
    hooks_json: Mapped[str] = mapped_column(Text, default="{}")  # 能力钩子配置,如 {"forbidden_words": [...]}
    manifest_json: Mapped[str] = mapped_column(Text, default="{}")  # 宿主协议 manifest 快照
    code_text: Mapped[str] = mapped_column(Text, default="")  # 插件代码(P0:内置插件为空)
    ui_slots_json: Mapped[str] = mapped_column(Text, default="[]")  # [{type, component, title}]
    enabled: Mapped[bool] = mapped_column(Boolean, default=True)  # 启用/停用(停用不卸载)
    scope: Mapped[str] = mapped_column(String(16), default="tenant")  # tenant | global
    source: Mapped[str] = mapped_column(String(16), default="builtin")  # builtin | custom
