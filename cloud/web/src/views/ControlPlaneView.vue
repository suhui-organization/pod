<template>
  <div>
    ...
    <el-alert type="info" :closable="false" show-icon class="hint"
      :title="t('控制平面事件：钩子、配置冻结项、记忆文件、MCP 包来源、Agent 身份、委托链、JIT 令牌、熔断。来自本机 pod posture / quarantine / delegate 等命令，经 pod sync 上云。')" />

    <div class="cards">
      <el-card shadow="never" class="card">
        <div class="card-num high">{{ summary.by_severity.high || 0 }}</div>
        <div class="card-label">{{ t('高危') }}</div>
      </el-card>
      <el-card shadow="never" class="card">
        <div class="card-num medium">{{ summary.by_severity.medium || 0 }}</div>
        <div class="card-label">{{ t('中危') }}</div>
      </el-card>
      <el-card shadow="never" class="card">
        <div class="card-num low">{{ summary.by_severity.low || 0 }}</div>
        <div class="card-label">{{ t('提示') }}</div>
      </el-card>
      <el-card shadow="never" class="card">
        <div class="card-num">{{ summary.total }}</div>
        <div class="card-label">{{ t('事件总数') }}</div>
      </el-card>
    </div>
    ...
    <el-table :data="events" v-loading="loading" stripe style="width: 100%">
      ...
      <el-table-column :label="t('级别')" width="90">
        <template #default="{ row }">
          <el-tag size="small" :type="severityType(row.severity)">{{ severityLabel(row.severity) }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column :label="t('决策')" width="100">
        <template #default="{ row }">
          <span :class="`dec-${row.decision}`">{{ row.decision }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="t('详情')" min-width="320">
        <template #default="{ row }">
          <div class="reason">{{ row.reason }}</div>
          <div class="meta">
            <span>{{ t('序号') }} #{{ row.seq }}</span>
            <code>{{ shortHash(row.hash) }}</code>
          </div>
        </template>
      </el-table-column>
    </el-table>
    <el-empty v-if="!loading && events.length === 0" :description="t('暂无控制平面事件（本机跑 pod posture 并 pod sync 后可见）')" :image-size="70" />

    <p class="footnote">
      {{ t('级别为服务端按事件类型与决策推导') }}（severity_source={{ severitySource }}），{{ t('用于快速分拣') }}；
      {{ t('原始事实是每行的事件类型、决策与哈希链') }}。
    </p>
  </div>
</template>
...
// computed 而不是普通常量：切语言时要立刻重算，否则标签停在初始化时的语言
const KIND_LABELS = computed(() => [
  { value: 'hook', label: t('生命周期钩子') },
  { value: 'config-change', label: t('配置冻结/漂移') },
  { value: 'memory', label: t('记忆完整性') },
  { value: 'package', label: t('MCP 包来源') },
  { value: 'identity', label: t('Agent 身份') },
  { value: 'delegation', label: t('委托链') },
  { value: 'grant', label: t('JIT 令牌') },
  { value: 'quarantine', label: t('熔断') },
  { value: 'anomaly', label: t('异常信号') },
  { value: 'metadata', label: t('工具元数据') },
])
...
function labelOf(k: string): string {
  return KIND_LABELS.value.find((i) => i.value === k)?.label ?? k
}
...
function severityLabel(s: string): string {
  if (s === 'high') return t('高')
  if (s === 'medium') return t('中')
  return t('低')
}
