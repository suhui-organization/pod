  ...
  <div class="settings">
    ...
    <el-card v-if="tenant" class="card">
      ...
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="计划">{{ tenant.plan }}</el-descriptions-item>
        <el-descriptions-item label="成员数">{{ tenant.members ?? '—' }}</el-descriptions-item>
      </el-descriptions>
    </el-card>
    ...
    <el-card id="ai-model" class="card" :class="{ 'card--focus': llmFocus }">
      ...
      <p v-if="llm.configured" class="llm-state">
        当前生效：<b>{{ providerLabel(llm.effective.provider) }}</b>
        <span class="llm-sep">·</span>
        <code class="mono">{{ llm.effective.model }}</code>
        <span v-if="llm.effective.key_source === 'env'" class="llm-tag">Key 来自环境变量</span>
      ...
      <p v-if="llm.configured" class="llm-sub">
        请求发往 <code class="mono">{{ llm.effective.endpoint }}</code>
      </p>
      ...
      <el-form v-if="auth.isAdmin" label-width="130px" class="llm-form">
        ...
        <el-form-item label="模型">
          <el-input
            ...
            style="width: 260px"
          />
          <span class="hint">留空用 Provider 默认{{ currentProvider?.default_model ? `（${currentProvider.default_model}）` : '' }}</span>
        </el-form-item>
        <el-form-item v-if="currentProvider?.needs_base_url" label="Base URL">
          ...
          <span class="hint">OpenAI 兼容端点，会自动补 <code class="mono">/chat/completions</code></span>
        </el-form-item>
        <el-form-item label="API Key">
          <el-input
            ...
            style="width: 420px"
          />
          <span class="hint">只写不回显；留空 = 保留已存的那把</span>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" :loading="llmSaving" @click="saveLlm">保存</el-button>
          <el-button :loading="llmTesting" @click="testLlm">测试连接</el-button>
          <span class="hint">测试用上面的表单（未保存也能测），会真实发一次请求并进审计</span>
        </el-form-item>
      </el-form>
      <p v-else class="hint">
      ...
      <p v-if="llmTest" class="llm-test" :class="llmTest.ok ? 'llm-test--ok' : 'llm-test--bad'">
        <template v-if="llmTest.ok">
          连通 · {{ llmTest.model }} · {{ llmTest.latency_ms }}ms · 返回「{{ llmTest.sample }}」
        </template>
        <template v-else>不通：{{ llmTest.error }}</template>
      </p>

      <div class="llm-deps">
        <div class="llm-deps__title">依赖这套配置的功能</div>
    ...
    <el-card class="card">
      <template #header>合规报告（GDPR）</template>
      <div class="report-row">
        <el-button type="primary" :loading="reportLoading" @click="downloadReport">生成 GDPR 报告（Markdown 下载）</el-button>
        <el-button :loading="reportLoading" @click="previewReport">查看摘要</el-button>
        <span v-if="reportSummary" class="report-summary">
          近 30 天 {{ reportSummary.activities.total_events }} 次调用 ·
          allow {{ reportSummary.activities.by_decision.allow ?? 0 }} / approve
          {{ reportSummary.activities.by_decision.approve ?? 0 }} / deny
          {{ reportSummary.activities.by_decision.deny ?? 0 }} ·
          审批 {{ reportSummary.activities.approvals.length }} 条
        </span>
      </div>
    </el-card>

    <el-card class="card">
      <template #header>告警通知</template>
      <el-form label-width="120px">
        <el-form-item label="启用推送">
          <el-switch v-model="wh.enabled" />
          <span class="hint" style="margin-left: 8px">高危/中危告警实时推送到群或自建接收端</span>
        </el-form-item>
        ...
        <el-form-item label="Webhook URL">
          ...
          <span v-if="wh.url_set && !wh.url" class="hint">已配置（URL 不回显，重新填写可覆盖）</span>
        </el-form-item>
        <el-form-item label="签名密钥">
          <el-input v-model="wh.secret" placeholder="可选，发送时放入 X-PodCloud-Signature 头" />
        </el-form-item>
        <el-form-item label="最低级别">
          <el-select v-model="wh.min_severity" style="width: 120px">
            <el-option label="高危及以上" value="high" />
            <el-option label="中危及以上" value="medium" />
            <el-option label="全部" value="low" />
          </el-select>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" :loading="whSaving" @click="saveWebhook">保存</el-button>
          <el-button v-if="wh.url_set" @click="testWebhook">发送测试告警</el-button>
        </el-form-item>
      </el-form>
      <p class="hint">支持渠道：企业微信 / 钉钉 / 飞书 / Slack / 自定义 JSON（generic）。规则 11 种：密钥拦截、敏感路径访问、策略不匹配、Deny 突增、调用风暴、注入信号、审批超时、未注册服务器、Agent 失联、人工批准、新工具首次使用。</p>
    </el-card>

    <el-card class="card">
      <template #header>邮件通知（SMTP，自行配置）</template>
      <el-form label-width="130px">
        ...
        <el-form-item label="账号">
          <el-input v-model="smtp.user" placeholder="alert@example.com（QQ/163 等用授权码）" style="width: 320px" />
        </el-form-item>
        <el-form-item label="密码/授权码">
          <el-input v-model="smtp.password" type="password" show-password placeholder="留空则保留已保存的密码" style="width: 320px" />
          <span v-if="smtp.user_set && !smtp.password" class="hint">已配置（不显示，重新填写可覆盖）</span>
        </el-form-item>
        <el-form-item label="发件人">
          <el-input v-model="smtp.from_addr" placeholder="alert@example.com" style="width: 320px" />
        </el-form-item>
        <el-form-item label="收件人">
          <el-input v-model="smtp.to_addrs" placeholder="ops@example.com, boss@example.com（逗号分隔）" style="width: 420px" />
        </el-form-item>
        <el-form-item label="TLS">
          <el-switch v-model="smtp.tls" />
          <span class="hint" style="margin-left: 8px">465 端口自动 SSL；其他端口走 STARTTLS</span>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" :loading="smtpSaving" @click="saveSmtp">保存</el-button>
        </el-form-item>
      </el-form>
    </el-card>

    <el-card class="card">
      <template #header>告警规则阈值</template>
      <el-form label-width="130px">
        <el-form-item label="Deny 突增阈值">
          <el-input-number v-model="rules.deny_burst_threshold" :min="1" :max="1000" style="width: 140px" />
          <span class="hint" style="margin-left: 8px">窗口内 deny 次数达到即告警</span>
        </el-form-item>
        <el-form-item label="统计窗口">
          <el-input-number v-model="rules.burst_window_seconds" :min="10" :max="86400" :step="10" style="width: 140px" />
          <span class="hint" style="margin-left: 8px">秒（deny 突增 / 调用风暴共用）</span>
        </el-form-item>
        <el-form-item label="调用风暴阈值">
          <el-input-number v-model="rules.spike_threshold" :min="2" :max="100000" style="width: 140px" />
          <span class="hint" style="margin-left: 8px">窗口内总调用数达到即告警（疑似失控循环）</span>
        </el-form-item>
        <el-form-item label="失联阈值">
          <el-input-number v-model="rules.silence_hours" :min="1" :max="720" style="width: 140px" />
          <span class="hint" style="margin-left: 8px">小时（agent 未同步即告警，巡检每 30 分钟一轮）</span>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" :loading="rulesSaving" @click="saveRules">保存</el-button>
    ...
    <el-card class="card">
      ...
      <el-form label-width="130px">
        <el-form-item label="每日推送">
          ...
          <span class="hint" style="margin-left: 8px">每天 09:00 推送昨日安全摘要到已配置的 webhook / 邮件（无告警也会推送一句确认）</span>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" :loading="digestSaving" @click="saveDigest">保存</el-button>
          <el-button :loading="digestPushing" @click="pushDigestNow">立即推送一份</el-button>
    ...
    <el-card class="card">
      ...
      <el-form label-width="120px">
        <el-form-item label="主题">
          <el-switch v-model="dark" active-text="暗色" inactive-text="亮色" @change="applyTheme" />
        </el-form-item>
...
async function testLlm() {
  ...
  try {
    ...
    llmTest.value = {
      ...
      endpoint: '',
      latency_ms: 0,
      error: e?.response?.data?.error?.message ?? '测试请求失败',
      sample: '',
    }
  } finally {
...
async function pushDigestNow() {
  ...
  try {
    ...
    const msg = e?.response?.data?.error?.message ?? '推送失败'
...
async function testWebhook() {
  ...
  ElMessage.success('测试告警已发送（如未收到请检查 webhook 地址与渠道格式）')
}
