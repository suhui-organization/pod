...
**管道**：先让两端知道彼此的版本与状态，再把"这台机器上有什么、扫出了什么"送上云。

下面每一节都是这一版的一部分（原先散在"未发布"里，发版时归拢到版本号下）。

### ② 资产 + ③ 发现：pod 干的活的另一半也上云了

一期分工是"pod 干活、podcloud 收集展示持久化"。但此前只有**行为**那一半（工具调用判定）
真的流到了云端：资产（这台机器上有什么）与发现（扫出了什么）都留在本地，
于是云端资产表只能靠人手工维护，Dashboard 也只有"干了多少活"，没有"还缺什么防护"。

### ② 资产清单（`POST /api/v1/sync/inventory`）

- 内容：harness 清单与纳管状态（identity / policy / audit）、MCP server 清单与
  **是否经过网关**、覆盖率、当前规则版本；
- 语义：**快照替换**——server 被删掉时云端跟着消失，不靠 diff；
- 落库：`pod_agent_assets`（一行一个资产），控制台 Agent 卡片显示
  "纳管 harness 2/4 · server 3 个，其中 1 个绕过网关"；
- Dashboard 新增**网关覆盖率**卡片：跨机器统计"还有多少个 server 绕过网关、
  涉及几台机器"——这才是运维每天要看的东西。

### ③ 发现（`POST /api/v1/sync/findings`）

- 内容：漏洞扫描（`guard`，key = AG-xx）与控制平面姿态（`posture`，key = 类别）
  的结果，按（威胁 × 级别 × harness）聚合成计数；
- 语义：同样是快照——修好后下次上报即消失，不会在云端永远留一条红；
- 落库：`pod_agent_findings`，Dashboard 新增**扫描发现**卡片（high / 全部 + Top 威胁）。

### 边界与兼容

- **隐私不变**：只出标识、布尔与计数——没有路径、args、env 取值、配置原文与证据；
  测试里有专门断言（摘要/资产/发现里都不出现本机路径与命令行）。
- **老服务端可用**：两个新端点返回 404 时只提示一次、不算同步失败
  （与 `/sync/quarantine` 同一口径）；老客户端不带这些 payload，云端照常工作。
- 顺手修掉一个展示口径问题：Dashboard 近 7 天趋势此前按**上传时间** `synced_at` 分组，
  离线机器补传会把历史事件全堆在"今天"；现在按**事件发生时间** `ts` 分组。

测试：`apps/cli/test/sync-protocol.test.ts` 9 项（新增资产/发现/404 容忍）、
`cloud/server/tests/test_sync_protocol.py` 8 项（新增落库、快照替换、鉴权、Dashboard 聚合）。

### 端 A ↔ 端 B 的契约层：协议版本 + 健康摘要
